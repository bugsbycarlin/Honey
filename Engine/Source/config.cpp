...
namespace Honey {
  ...
  // Use this interval
  void Config::setUpdateInterval(float interval) {
    update_interval = interval;
    timing.remove("config_update");
    timing.lock("config_update", update_interval);
    update();
  }

  // Check the time and update
  int Config::checkAndUpdate() {
    if (!timing.locked("config_update")) {
      timing.lock("config_update", update_interval);
    } else {
      return SLEEPING;
    }
  ...
  int Config::update() {
    ...
    if (result != XML_SUCCESS) {
      printf("Config load failed entirely! Failed to load and parse file at path %s\n", path.c_str());
      return FAILURE;
    }
    ...
    while (section != nullptr) {
      ...
      if (node_value != "section") {
        printf("Config load failed on line number %d. Top level nodes must have type 'section'. Found %s instead.\n", section->GetLineNum(), node_value.c_str());
        return FAILURE;
      }

      const char* section_name_chars;
      if (section->QueryStringAttribute("name", &section_name_chars) != XML_SUCCESS) {
        printf("Config load failed on line number %d. Section is missing name.\n", section->GetLineNum());
        return FAILURE;
      }
      ...
      while (element != nullptr) {
        ...
        if (node_value != "param") {
          printf("Config load failed on line number %d. Nodes inside sections must have type 'param'. Found %s instead.\n", element->GetLineNum(), node_value.c_str());
          return FAILURE;
        }

        const char* element_type_chars;
        if (element->QueryStringAttribute("type", &element_type_chars) != XML_SUCCESS) {
          printf("Config load failed on line number %d. Param is missing type.\n", element->GetLineNum());
          return FAILURE;
        }
        ...
        if (element->QueryStringAttribute("name", &element_name_chars) != XML_SUCCESS) {
          printf("Config load failed on line number %d. Param is missing name.\n", element->GetLineNum());
          return FAILURE;
        }
        ...
        if (element_type == "bool") {
          ...
          if (value != "true" && value != "false") {
            printf("Config load failed on line number %d. Failed to parse bool value. Expected true or false, but got %s.\n", element->GetLineNum(), value.c_str());
            return FAILURE;
          } else if (value == "true") {
          ...
          if (result != XML_SUCCESS) {
            printf("Config load failed on line number %d. Failed to parse int value.\n", element->GetLineNum());
            return FAILURE;
          }
          ...
          printf("Config load failed on line number %d. Unknown type %s. Must be bool, int, float, or string.\n", element->GetLineNum(), element_type_chars);
          return FAILURE;
        }
  ...
  bool Config::getBool(string section, string name) {
  ...
  int Config::getInt(string section, string name) {
  ...
  float Config::getFloat(string section, string name) {
  ...
  string Config::getString(string section, string name) {
